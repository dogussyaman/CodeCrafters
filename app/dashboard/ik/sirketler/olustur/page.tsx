...
export default function CreateCompanyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Şirket oluşturulurken bir hata oluştu")
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-2xl min-h-screen">
      <div className="mb-6">
        <BackLink href="/dashboard/ik/sirketler" />
      </div>

      <Card>
        <CardHeader>
          <CardTitle className="text-2xl">Yeni Şirket Ekle</CardTitle>
          <CardDescription>Şirket bilgilerini girerek başlayın</CardDescription>
        </CardHeader>
        <CardContent>
